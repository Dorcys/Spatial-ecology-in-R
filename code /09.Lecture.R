...
library(terra)
library(imageRy)       
# in imagery there is a function im.list()
im.list()              
# lists all the images we can use
...
sunc <- im.classify(sun , num_clusters=4)       
# sun classified (sunc) - function is im.classify(image, number of clusters)
plot(sunc)                                      
# need new version of imageRy - plots same graph 3 or 4 times
...
f1992 <- freq(m1992c)
f1992                     
# we get table of frequencies
# 2006  
f2006 <- freq(m2006c)
f2006
# percentages
tot1992 <- ncell(m1992)
# ncel calculating number of pixels in image
#1800000 - total number of pixels
perc1992 <-  f1992 * 100 / tot1992     
# frequency of 1992 * 100 divided by total number of pixels
perc1992                  
# look under count  
# 1992: forest = 83.08%, agriculture = 16.91%
tot2006 <- ncell(m2006)    
# frequency of the image of 2006 
# 7200000
...
p1+p2         
# if we want to see the plots one next to each other 
