...
library(overlap)     
# https://cran.r-project.org/web/packages/overlap/vignettes/overlap.pdf  we are using part of the vignette - and we will build some of those graphs
# relation among species in space - use multivariate analysis (previous lecture)
# relation among species in time 
# data
data(kerinci)
head(kerinci)       
# first 6 rows - it shows zone - species - time 
kerinci             
# to see the whole dataset 
summary (kerinci)   
# general information about the dataset - we dont need it its just to see
# let's see how species 'tiger' has moved throughout the day 
# tiger
tiger <- kerinci[kerinci$Sps == "tiger",]       
# now we need the comma at the end of parenthesis to end 
# in sequel - final symbol is ; but in R is just comma , 
tiger                                           
# new data set with only data connected to "tiger" 
...
# add new colum with this calculation 
...
head(kerinci)       
# new column aded - time passed to radians (ratio between a line and radius of a circumfernece) 
# now remake tiger 
tiger <- kerinci[kerinci$Sps == "tiger",] 
head(tiger)                        
# new dataset tiger with additional column 
# final plot of the "tiger" 
plot(tiger$timeRad)                
# plot tiger and time radians 
timetig <- tiger$timeRad           
# assigning the tiger and time to timetig 
densityPlot(timetig, rug=TRUE)     
# function showing amount of time in analysis 
