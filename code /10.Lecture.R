...
# 4th band is a control band we dont use 
...
# enhance vegetaion in green - you can see all the rocks in violet 
# now calculate variability - can be only on one variable (ex.age)
# let's use only in near infra red (NIR)
nir <- sent[[1]]        
# nir = first element of the 'sent' image 
plot(nir)               
# near infra red - green parts vegetation, bare soil is pinkish 
...
# 7 colors and 255 amount of different inner-colors used 
plot(sd3, col=viridis)                              
# we can see what is the probability in space of near infra red - ultra complex geology is green - very dangerous in glaciar environment
...
# final number of pixels is now 49 so 1/49 
plot(sd7, col=viridis)                               
# we can now see more 
# Exercise: plot together via par(mfrow()) the 3x3 and the 7x7 standard deviation
together <- par(mfrow=c(1,2))
plot(sd3, col=viridis)           
# very local calculation if standard deviation is 3x3 - in infrared only suble differences
plot(sd7, col=viridis)           
# if window enlarged including additional pixels (49 now) - bigger moving window, higher variability in this image 
